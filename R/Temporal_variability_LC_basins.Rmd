...
Some parameters were not measured for the whole period. _`r length(which(!is.na(mysubset[,parameter])))`_ are available for the above parameters.

Parameters to chose from are:  
   -`r paste(names(total)[-c(1:2)],collapse = "\n   -")`
   
```{r match gps data, include=FALSE}
length(which(!is.na(mysubset[,parameter])))
