...
LTdiet %>% mutate(group = case_when(Clip.Location == "NC" ~ "Wild",
...
LTdiet %>% ggplot(mapping = aes(x = Total.Length, y = as.numeric(Stomach.Weight))) +
  geom_point() + 
  xlab("Length (mm)") + 
...
LTdiet %>% ggplot(mapping = aes(x = Weight, y = as.numeric(Stomach.Weight))) +
  geom_point() + 
  xlab("Weigth (g)") + 
  ylab("Stomach Weight (g)") +
  stat_smooth(method = "lm") 
