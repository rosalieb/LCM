...
p2 <- ggplot() +  geom_polygon(data=bathy, aes(x=long, y=lat, group=group, fill=lcgroupmap), show.legend = FALSE) +
  ...
  #geom_point(data=LTeff_coord, aes(x=start.lon.dd, y=start.lat.dd), color="red") +
...
p2 <- ggplot() +  geom_polygon(data=bathy, aes(x=long, y=lat, group=group, fill=lcgroupmap), show.legend = FALSE) +
  scale_fill_manual(values = c("white","darkgrey")) +
#  geom_point(data=LTeff_coord[LTeff_coord$date==choose_day,], aes(x=start.lon.dd, y=start.lat.dd), color="red") +
  xlab("Longitude") + ylab("Latitude") + theme_minimal()  +
  theme_set(theme_bw()) +
  #ylim(44.3,44.4) +
  coord_equal(ratio=1) # square plot to avoid the distortion

p2 <- p2 + geom_segment(aes(x = LTeff_coord$start.lon.dd[LTeff_coord$date==choose_day], y = LTeff_coord$start.lat.dd[LTeff_coord$date==choose_day], xend = LTeff_coord$end.lon.dd[LTeff_coord$date==choose_day], yend = LTeff_coord$end.lat.dd[LTeff_coord$date==choose_day]))
