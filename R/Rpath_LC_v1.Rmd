...
_Copied from <a href="https://rdrr.io/github/NOAA-EDAB/Rpath/f/vignettes/Rpath.Rmd"> here</a>, I don't know how stable the link is._
<br>
...
```{r parameter file generation lake champlain}
groups <- c('Sea Lamprey', 'Adult lake trout', 'Juvenile lake trout', 'Stocked lake trout', 'Atlantic salmon', 'Burbot', 'Walleye', 'Whitefish', 'Cisco', 'Smelt', 'Alewife', 'Trout-perch', 'Sculpin', 'Zoo predators', 'Mysis', 'Benthic invertebrates', 'Zoo grazers', 'Phytoplankton', 'Detritus')
# type 1= primary producer
# type 2= detritus
# type 3= fleet
# type 0= consumer (the rest)
types  <- c(rep(0, length(groups)-2), 1, 2)

stgroups <- c(rep(NA, 1), rep('LakeTrout', 3), rep(NA, 15))

#REco.params <- create.rpath.params(group = groups, type = types, stgroup = stgroups)
```

### Model parameters
Not run at the moment -- still need to get Rpath first.
