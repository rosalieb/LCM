...
REco.params.LC$model[Group %in% c('Cisco'), EE := .1] # no data
...
```{r}
# REco.params.LC$model$Biomass[2] =85
# REco.params.LC$model$EE[2] =.8
# REco.params.LC$model$Biomass[4] =4
# REco.params.LC$model$EE[4] =NA
# REco.params.LC$model$Biomass[18] =700
# REco.params.LC$model$EE[18] =NA
...
V(net_w)$color[order(ewediet_nodes_w$level)] <- colrs
...
if(!R_U_KNITTING) pdf(paste0(getwd(),"/Output/Figures/6-Rpath/summary_interaction1.pdf"),width = 8, height = 8)
par(mar=c(1,4,0,5))
par(xpd=T)
...
Rp1 <- rsim.scenario(REco2, Rpath.params = REco.params.LC, years = 1:100)
Rs1 <- rsim.run(Rp1)

rsim.plot(Rs1, spname = Rp1$params$spname)


adjust.forcing(Rp1, forcing, group = "Cisco", sim.year = 10:80, sim.month = 0,
  bymonth = F, sample(seq(1,4,length.out = 30), size = 81))
```
