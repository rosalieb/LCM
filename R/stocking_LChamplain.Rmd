...
```{r}
ggplot() +
  geom_line(data=summ_sp_yr, aes(x=as.numeric(Year),y=BNT, linetype=State), col="darkred", size=1.5) +
  geom_line(data=summ_sp_yr, aes(x=as.numeric(Year),y=LAS, linetype=State), col="#E69F00", size=1.5) +
  geom_line(data=summ_sp_yr, aes(x=as.numeric(Year),y=LAT, linetype=State), col="#56B4E9", size=1.5) +
  geom_line(data=summ_sp_yr, aes(x=as.numeric(Year),y=STT, linetype=State), size=1.5) + 
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9", "black"), 
                       name="Species",
                       breaks=c("BNT", "LAS", "LAT", "STT"),
                       labels=c('Brown Trout','Landlocked Atlantic Salmon', 'Lake Trout','Steelhead Trout')) +
  xlab("Species") + ylab("total # stocked") +
  labs(fill="Year") + ggtitle("Lake Champlain waters")
