...
```{r Model some response variables in smelt population against drivers - step 4: look at the shape of some relationship}
...
for (i in resp_smelt) {
  for (l in 1995:2010) {
      ...
      for (k in 0:length(Station)) {
          ...
          ggplot(smelt_mod_subset, aes(Year, resp)) + geom_point(aes(shape=periods)) + stat_smooth(method = "lm") + labs(y = colnames(smelt_mod)[i]) + theme_bw() +
            stat_smooth(data, aes)
      }
    }
}
# Clean output
