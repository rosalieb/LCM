...
ewediet$predator <- gsub("Lamprey", "Sea Lamprey", ewediet$predator)
...
ewediet_nodes <- data.frame(
  ...
  "data_source"=as.character(c("VTFWS", "VTFWS/UVM","VTFWS/UVM","VTFWS/NYFWS","NA","NA","NA","UVM","UVM","UVM","UVM","UVM","UVM","VTDEC","VTDEC","VTDEC","UVM","VTDEC","calculated")),
  "level"=c(-2,0,3,3.5,0,0,0,1,0,5,4,2,5,7,6,7,6.5,8,9)
)
...
par(xpd=T)
TeachingDemos::shadowtext(-norm_coords(mylayout[,1:2], -1, 1, -1, 1), label=gsub("Zoo","Zoo.",ewediet_nodes$id),pos=mylayout[,3], offset=1.3, col="black",bg = "white",  theta = (1:8/4)*pi, r1 = 0.06, r2 = 0.04)
par(xpd=F)
dev.off()
...
summary_ls <- list(
  ...
  "Mysis" = mysis_summ[,c("Year","mean_w")],
  ...
  "Walleye" = NA,
  "stocked lake trout" = data.frame("Year"=c(1973:2008,as.numeric(paste(summ_stkvt$Year))), "Stocked"=c(rep(max(summ_stkvt$`Total #fish stocked`),length(c(1973:2008))),summ_stkvt$`Total #fish stocked`)),
...
par(mar=c(5,7,4,0.5))
plot(c(1973,2019), c(1,length(summary_ls)), pch=NA, xlab="Year", ylab="", axes=F)
...
for (i in 1:length(summary_ls)) {
  ...
  TeachingDemos::shadowtext(x=1973,y=i, labels = names(summary_ls[i]), col = "black",bg = "white", theta = seq(pi/4, 2 * pi, length.out = 8), r = 0.1, pos=2)
  par(xpd=F)
