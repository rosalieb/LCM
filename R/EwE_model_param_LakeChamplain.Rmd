...
pdf(paste0(getwd(),"/Output/Figures/5-Summary/summary_interaction4.pdf"),width = 8, height = 8)
par(mar=c(0,4,0,7))
...
secchi_depth_p
```
<br>`r fig_cap("secchi", caption = "Secchi depths across all sites and years")`
...
dtlcm_secchi <- dtlcm[,c("StationID", "VisitDate", "Secchi.Depth")]
...
p1 <- ggplot(dtlcm_secchi_summ2[dtlcm_secchi_summ2$Year!=2015,],aes(x=meiv_summer, y=value, col=Year)) +
...
```
`r fig_cap("mysis biomass 1", caption="Average (and sd) mysis biomass per year")`
...
Below, I wrote a function to compute Fulton's factor ($K = W/L^3$) for different bin sizes. The function is called ```Compute_FultonK``` and require the following arguments: `r args(Compute_FultonK)`

```{r function calculate fulton factor for different bin size, message=FALSE, warning=FALSE, include=FALSE}
Compute_FultonK <- function(pop_ID=NULL, pop_length, pop_weight, bin_limits, outliers = NULL) {
    ...
    for (i in 2:length(bin_limits)) {
      ...
      p2 <- p2 + geom_point(data =list_out$Data[which_in_bin,],mapping = aes(x=log(pop_length), y=log(pop_weight)), colour=colrs[i-1])
    ...
    aplot <- ggplot(param, aes((param$Min_size+param$Max_size)/2, a)) + geom_point() + stat_smooth() +xlab("Average size (mm)")
    bplot <- ggplot(param, aes((param$Min_size+param$Max_size)/2, b)) + geom_point() + stat_smooth() +xlab("Average size (mm)")
...
Below, I wrote a function to compute relative condition factor.
$K_n = W/W'$
...
The function is called ```RelCond_Fisheries``` and require the following arguments: `r args(RelCond_Fisheries)`

```{r function calculate relative fulton factor for different bin size, message=FALSE, warning=FALSE, include=FALSE}
RelCond_Fisheries <- function(pop_ID=NULL, pop_length, pop_weight, size_limits, outliers = NULL) {
     # Create list to save outputs
    list_out <- list()
...
Fult3$plot_KL
mean(Fult3$Data$delta_to_K, na.rm=T)
Fult4 <- Compute_FultonK(pop_ID = LTcond$Trawl.Id[which(LTcond$tl.mm>100 & LTcond$tl.mm <400)],
                ...
                pop_length = LTcond$tl.mm[which(LTcond$tl.mm>100 & LTcond$tl.mm <400)],
...
ggplot(list_out$Data, aes(pop_length,FultonK, fill=size_group)) + geom_point()
  xlab("Length (mm)") + ylab("K")
...
LTcond$Condition
plot(LTcond$Condition ~ LTcond$frozen.tl,xlab="total length (mm)",ylab="K",main="", pch=20)
lm3 <- lm(LTcond$Condition ~ as.numeric(paste(LTcond$frozen.tl)),data=LTcond)
abline(lm3, col="red", lwd=1.5)
mtext("a. Slope ≠ 0", side = 3, line=1.5)
...
trap_summ2 <- data.frame("Total.catch"=colSums(trap_summ,na.rm = T)[order(colSums(trap_summ,na.rm = T), decreasing = T)])

ggplot(trap_summ2, aes(rownames(trap_summ2),Total.catch)) +
  geom_bar(stat="identity", position=position_dodge()) + ylab("Number lamprey caught") + xlab("Stations")
...
# create a summary
stklkt_summ <- as.data.frame(with(stklkt, tapply(NB_EQUIV,list("Stocking_site"=STOCKING_SITE, "Year"=YEAR), sum, na.rm=T)))
stklkt_summ$TOTAL <- rowSums(stklkt_summ, na.rm = T)
stklkt_summ <- rbind(stklkt_summ, "TOTAL"=colSums(stklkt_summ, na.rm = T))
stklkt_summ <- rbind(stklkt_summ, "PERCENT"=stklkt_summ["TOTAL",]/stklkt_summ["TOTAL", "TOTAL"])*100


```

```{r plot stoking lkt data long term, echo=FALSE, message=FALSE, warning=FALSE}
p1 <- ggplot(data=as.data.frame(t(stklkt_summ[!(colnames(stklkt_summ) %in% "TOTAL")])), aes(x=as.numeric(paste(colnames(stklkt_summ)[! (colnames(stklkt_summ) %in% "TOTAL")])),y=TOTAL))+
  geom_bar(stat="identity") + xlab("Year") + ylab("Numer of yearling-equivalent lake trout") 
...
summary_ls <- list(
  ...
  "Walleye" = NA,
  "Stocked juveniles lake trout" = data.frame("Year"=1972:2016, "Stocked"=as.vector(t(stklkt_summ["TOTAL",as.numeric(paste(colnames(stklkt_summ))) %in% 1972:2016]))),
...
pdf(paste0(getwd(),"/Output/Figures/5-Summary/summary_biomass2.pdf"),width = 12, height = 6)
