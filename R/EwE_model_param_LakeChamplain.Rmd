...
The secchi depth across all sites and years is ploted below (`r fig_cap("secchi", display="cite", levels=2)`).
...
eu_z = 4.9
```
`r fig_cap("secchi", caption = "Secchi depths across all sites and years")`
...
( tote_troutperch =      40         *         8           *        4          )

```


Now that we somewhat have an estimation of number per tote, convert that back and get an idea of the average abundance.



It'd be easier to assign the GPS coordinates to each date to be sure there's not a big spot with / without trout-perch.

```{r match gps coordinates trawls for trout perch surveys, message=FALSE, warning=FALSE, include=FALSE}
# Read LTeff compiled
