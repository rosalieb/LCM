...
smelt_tot_stats <- smelt_tot_stats %>%
  mutate(variable2 = case_when(
    ...
    variable == "mean_l" ~ "length (mm)",
    variable == "mean_w" ~ "weight (g)",
    variable == "mean_cond" ~ "condition",
    variable == "mean_age" ~ "mean age"
  ))

smelt_tot_stats$variable2 <- factor(smelt_tot_stats$variable2,
                                    levels = c('log(CPUE)' ,'length (mm)' ,'weight (g)' ,'condition', 'mean age'))
...
#ggsave(paste0(getwd(),"/Output/Figures/9-Smelt/Final_figure_summary.png"), width = 6, height=7)
```
