...
Below, I wrote a function to compute Fulton's factor for different bin sizes. The function is called ```Compute_FultonK``` and require the following arguments: `r args(Compute_FultonK)`
...
Compute_FultonK <- function(pop_ID=NULL, pop_length, pop_weight, bin_limits, outliers = NULL) {
    ...
    list_out$Data <- data.frame("row_nb"=1:length(pop_length),pop_ID, pop_length,pop_weight)
    ...
    for (i in 2:length(bin_limits)) {
      ...
      if (length(which_in_bin)>1) {
        lm_temp = lm(log(pop_weight[which_in_bin])~log(pop_length[which_in_bin]))
      # fitPlot(lm1,xlab="log total length (mm)",ylab="log weight (g)",main="")
      a_temp = exp(lm_temp$coefficients[1])
