...
p1 <- ggplot(smelt_trawl, aes(Year, CPUE)) + 
  ...
  geom_smooth(color="tomato") +
  geom_vline(xintercept = 2003, lty=2) +
  geom_point(aes(shape=smelt_trawl$Station)) +
  scale_shape_manual(values=c(16,4,16,16,1), show.legend = F) +
  theme_bw() +
...
```{r Model some response variables in smelt population against drivers - step 1: prepare drivers}
# Drivers
# Mysis biomass
...
drivers<- merge(stklkt_summ2 , mysis_summ, by="Year")
...
```{r Model some response variables in smelt population against drivers - step 2: get the drivers + response dataframe clean from previous step}
drivers_smelt <- merge(drivers)
for(i in Station) {
...
drivers_smelt <- merge(drivers_smelt,dtlcm_smelt, by="Code")
...
```{r Model some response variables in smelt population against drivers - step 3: actual model}
colnames(smelt_mod)
# Get two vectors with col numbers of drivers and responses, so I can model everything against everything
   resp_smelt <- c(2:4)
drivers_smelt <- c(5:7,13:28)
...
for (i in resp_smelt)  {
  for (j in drivers_smelt) {
    for (k in 0:length(Station)) {
      ...
      # Linear model ####
      lm1 <- lm(smelt_mod_subset[!is.na(smelt_mod_subset[,i]) & !is.na(smelt_mod_subset[,j]),i] ~ smelt_mod_subset[!is.na(smelt_mod_subset[,i]) & !is.na(smelt_mod_subset[,j]), j], data = smelt_mod_subset)
    
      # save output
      out_mod_lm <- c(out_mod_lm, 
                      "lm", 
                      ifelse(k!=0, Station[k], "All lake"),
                      colnames(smelt_mod_subset)[j], colnames(smelt_mod_subset)[i], summary(lm1)[[8]], summary(lm1)[[4]][2,4], j, i)
    
      # GAM ####
      if(length(smelt_mod_subset[!is.na(smelt_mod_subset[,i]) & !is.na(smelt_mod_subset[,j]),i])>18) {
        gam1 <- gam(smelt_mod_subset[!is.na(smelt_mod_subset[,i]) & !is.na(smelt_mod_subset[,j]),i] ~ s(smelt_mod_subset[!is.na(smelt_mod_subset[,i]) & !is.na(smelt_mod_subset[,j]), j]), data = smelt_mod_subset[!is.na(smelt_mod_subset[,i]) & !is.na(smelt_mod_subset[,j]),])
        ...
        out_mod_gam <- c(out_mod_gam,
                     "gam", 
                      ifelse(k!=0, Station[k], "All lake"),
                     colnames(smelt_mod_subset)[j], colnames(smelt_mod_subset)[i],
        ...
        if(gam1_summ$dev.expl>0.3) {
          plot.gam(gam1, xlab = colnames(smelt_mod_subset)[j], 
             ylab = paste0("s(", colnames(smelt_mod_subset)[i], ")"),
...
```{r code from lake geneva paper eval=FALSE, include=FALSE}
# code from Lake Geneva paper
colnames(smelt_mod)
   resp_smelt <- c(2:4)
drivers_smelt <- c(5:7,13:28)
...
i = 3 #CPUE 
j = 6 # total number of fish stocked
windowtest_residuals <- NULL
for (l in 2008) {
  sep= l
  for (k in 0:length(Station)) {
      # Run the model for lake, per stations, and per basin
  ...
  Results <- c(smelt_mod_subset$Year[i],info$Pr[2],signif(info$Pr[2]),info$Pr[3], signif(info$Pr[3]),model41$coefficients[2],info41$adj.r.squared, model42$coefficients[2],info42$adj.r.squared,smelt_mod_subset$Year[i])
  windowtest_residuals <- rbind (windowtest_residuals, Results)
}
colnames(windowtest_residuals) <- c("SplitingYear","AOV_pvalue_SATA","Significativity_SATA","AOV_pvalue_Periods","Significativity_Periods","SlopeAnthrophocene","rsquaredAnthropo","SlopePristine","rsquaredPristine","Year")
