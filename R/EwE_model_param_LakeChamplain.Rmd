...
p2 <- ggplot(smelt, aes(x=Age)) + 
  ...
  geom_bar(aes(y = (..count..)/sum(..count.. ~Basin2))) + 
...
#ggsave(paste0(getwd(),"/Output/Figures/9-Smelt/Smelt_logCPUE_per_basin2.png"), width = 3, height=5.5)
...
p3 <- ggplot(smelt_trawl_w, aes(Year, log(CPUE))) + 
  ...
  facet_wrap(~Basin, ncol = 1,
    ...
    sec.axis = sec_axis(~ ., name = "log(CPUE)"))  
  # scale_y_continuous(sec.axis = sec_axis(~ exp(.), name = "CPUE", breaks = c(0,10,35,120,400,1500,5000)))
#ggsave(paste0(getwd(),"/Output/Figures/9-Smelt/Smelt_logCPUE_per_basin_running_average2.png"), width = 3, height=5.5)

library(ecp)
for (i in seq_along(unique(smelt_trawl_summ$Basin))) {
  changepoint_found <- 
    e.divisive(as.matrix(smelt_trawl_summ[smelt_trawl_summ$Basin==unique(smelt_trawl_summ$Basin)[i],"CPUE"]), min.size = 3, sig.lvl = .7)$order.found[-c(1:2)]
